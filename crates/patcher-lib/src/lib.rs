...
mod diff {
    ...
    pub fn diff_in_tar(old: &Path, new: &Path, destination: &mut tar::Builder<impl Write>, mut update: impl FnMut(DiffState)) -> Result<(), DiffError> {
...
#[cfg(feature = "patch")]
mod patch {
    use std::{fs::File, io::{self, BufReader, BufWriter, Read}, path::{Path, PathBuf}};
    use thiserror::Error;
    ...
    pub fn patch_from_tar(old: &Path, new: &mut tar::Archive<impl Read>, destination: &Path, mut update: impl FnMut(CurrentPatchingPath)) -> Result<(), PatchError> {
        if old.is_dir() && (destination.is_dir() || !destination.exists()) {
            std::fs::create_dir_all(&destination)?;
            for file in new.entries()? {
                let file = file?;
                (update)(CurrentPatchingPath { path: file.path()?.into_owned() });
